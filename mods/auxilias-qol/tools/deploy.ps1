param(
    [string]$Destination = 'C:\Users\USER\Zomboid\Workshop\AuxiliasQoL'
)
...
if ($destinationName -ne 'AuxiliasQoL' -or [string]::IsNullOrWhiteSpace($destinationParent)) {
    throw "Deployment target must be a dedicated AuxiliasQoL directory: $resolvedDestination"
}
...
& $validateScript
Assert-NoReparsePoint -Root $source

New-Item -ItemType Directory -Path $destinationParent -Force | Out-Null
$resolvedParent = (Resolve-Path -LiteralPath $destinationParent).Path
$stagingPath = Join-Path $resolvedParent ('.auxilias-qol-stage-' + [guid]::NewGuid().ToString('N'))
$backupPath = Join-Path $resolvedParent ('.auxilias-qol-backup-' + [guid]::NewGuid().ToString('N'))
foreach ($managedPath in @($normalizedDestination, $stagingPath, $backupPath)) {
    $fullManagedPath = [System.IO.Path]::GetFullPath($managedPath)
    if (-not [System.IO.Path]::GetDirectoryName($fullManagedPath).Equals($resolvedParent, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Deployment path escapes the dedicated destination parent: $fullManagedPath"
    }
}
$movedExisting = $false
...
Write-Host "Deployed and verified Auxilia's Quality of Life at $normalizedDestination"
